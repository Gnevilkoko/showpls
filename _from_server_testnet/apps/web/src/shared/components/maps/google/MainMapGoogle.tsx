...
interface TaskMarkerContentProps {
  title: string
  price: number
...
const taskMarkerTitle = (raw: string) => {
  const title = (raw || "").trim()
  if (!title) return "Задача"
  return title.length > 18 ? `${title.slice(0, 17)}…` : title
}

const taskPriceText = (price: number) => `${Math.max(0, Math.round(Number(price) || 0))} ★`

const TaskMarkerContent = ({ title, price, urgentHours, accent }: TaskMarkerContentProps) => {
  return (
    <div className={`custom-marker ${accent ? "accent" : ""}`}>
      <div className="custom-marker__content">
        <strong>{taskPriceText(price)}</strong>
      </div>
      <div
        style={{
          marginTop: 4,
      ...
      >
        {taskMarkerTitle(title)}
      </div>
      {urgentHours != null && (
        <div style={{ color: "#ffeb3b", fontSize: 10, fontWeight: 700, marginTop: 2 }}>⏱ {urgentHours}ч</div>
      )}
    </div>
  )
}

function buildTaskMarkerIcon(task: TaskType): google.maps.Icon {
  const W = 148
  const H = 50
  ...
  ctx.fillText(taskPriceText(task.price), 10, 18)

  ctx.fillStyle = "#eceff1"
  ctx.font = "600 11px system-ui, sans-serif"
  ctx.fillText(taskMarkerTitle(task.title), 10, 35)

  const hTag = task.tags.find((t) => t.type === "hLeft")
  if (hTag) {
    ctx.fillStyle = "#ffeb3b"
    ctx.font = "700 10px system-ui, sans-serif"
    ctx.fillText(`⏱ ${hTag.count}ч`, 92, 18)
  }
          ...
          tasksList.forEach((task) => {
            ...
            root.render(
              <TaskMarkerContent
                ...
                urgentHours={task.tags.find((t) => t.type === "hLeft")?.count ?? null}
                accent={task.mode === "pro"}
              />
            )
