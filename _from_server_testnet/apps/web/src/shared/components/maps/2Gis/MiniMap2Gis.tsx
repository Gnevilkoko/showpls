...
interface MiniMap2GisProps {
  ...
  /**
   * Прямой геокод 2GIS только при явном «применить» (например blur инпута), тик > 0.
   * Без тика запросы к catalog.api.2gis.com не шлются — дебаунс по строке давал лавину вызовов.
   */
  geocodeApplyTick?: number
}

const MiniMap2Gis = memo(({ address, onCoordinatesChange, geocodeApplyTick = 0 }: MiniMap2GisProps) => {
  ...
  // Прямой геокод только при инкременте geocodeApplyTick; address читаем из ref (не триггерить на каждый символ)
  useEffect(() => {
    if (geocodeApplyTick === 0) return
    const trimmed = addressRef.current.trim()
    if (!trimmed) return
    void geocodeAddress(trimmed)
  }, [geocodeApplyTick, geocodeAddress])
