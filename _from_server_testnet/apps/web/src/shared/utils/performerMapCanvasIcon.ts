...
/** Внешние аватарки без CORS — через наш API, чтобы canvas не оставался без фото. */
function toSameOriginAvatarUrlForCanvas(resolvedAbsolute: string): string {
  if (typeof window === "undefined") return resolvedAbsolute
  try {
    const abs =
      resolvedAbsolute.startsWith("http://") || resolvedAbsolute.startsWith("https://")
        ? resolvedAbsolute
        : new URL(resolvedAbsolute, window.location.origin).href
    if (new URL(abs).origin === window.location.origin) return abs
    return `${window.location.origin}/api/upload/map-image?u=${encodeURIComponent(abs)}`
  } catch {
      ...
      p.avatar.startsWith("http") || p.avatar.startsWith("//") || p.avatar.startsWith("/")
        ? p.avatar.startsWith("/")
          ...
          : p.avatar.startsWith("//")
            ...
            : p.avatar
        : p.avatar
    const img = await loadImageForCanvas(toSameOriginAvatarUrlForCanvas(src))
    if (img) {
      try {
        ctx.save()
        ctx.beginPath()
