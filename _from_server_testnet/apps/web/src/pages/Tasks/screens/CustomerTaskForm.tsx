...
import { useCreateDirectRequestMutation, useCreateRequestMutation, type CreateRequestInput } from "../../../store/api/requestApi"
...
const CustomerTaskForm = ({ callback }: { callback: () => void }) => {
  ...
  const [createDirectRequest, { isLoading: isCreatingDirect }] = useCreateDirectRequestMutation()
  ...
  const uploadAttachments = useCallback(
    async (attachments: UploadedImageType[]): Promise<string[]> => {
      const urls: string[] = []
      // Последовательно: меньше одновременной нагрузки на API/S3 и шанс 502/503 у прокси
  ...
  const handleSendDirectOffer = useCallback(
    async (performer: PerformerNearby) => {
      if (String(performer.id) === String(myUserId ?? "")) {
        notification.showError("accessDenied")
        return
      }
      if (!validation.isValid) {
        showWarningOnFields()
      ...
      try {
        ...
        const deadlineAt = calculateDeadlineAt()
        const requestData = prepareRequestData(attachmentUrls, deadlineAt)
        const created = await createDirectRequest({
          ...requestData,
          performerId: String(performer.id),
        }).unwrap()
        setIsOpenPerformerDiscover(false)
        toast.success(`Предложение отправлено исполнителю ${performer.firstName}`)
        navigate(`/chat/${created.chatId}`)
      } catch (error: unknown) {
        const uploadErr = error as {
          status?: string | number
          error?: string
          originalStatus?: number
        }
        const isUploadError =
          uploadErr.status === "PARSING_ERROR" ||
          uploadErr.error === "SyntaxError" ||
          uploadErr.originalStatus === 503 ||
          (typeof uploadErr.status === "number" && uploadErr.status >= 500)
        if (isUploadError) {
          return
        }

        const apiErr = error as { data?: { message?: string; errorCode?: string } }
        const code = apiErr?.data?.errorCode || apiErr?.data?.message
        if (code === "PERFORMER_UNAVAILABLE") {
          NotificationHandler.showError(error as APIError, "Исполнитель сейчас недоступен. Выберите другого.")
          return
        }
        if (NotificationHandler.isAPIError(error)) {
          NotificationHandler.showError(error as APIError)
  ...
  return (
    <>
      ...
      <Modal isOpen={isOpenPerformerDiscover} onClose={handleClosePerformerDiscover}>
        <PerformersMapGoogle onSendOrder={handleSendDirectOffer} sendDisabled={isCreatingDirect} />
      </Modal>
    </>
  )
}
