...
const baseQueryWithAuth = fetchBaseQuery({
  baseUrl: "/api",
  credentials: "include",
  // Uploads (S3) and heavy mutations can exceed 20s behind reverse proxies
  timeout: 120000,
  ...
  if (result.error && result.error.status === 401) {
    ...
    if (!isRefreshing) {
      ...
      const runRefresh = async () => {
        try {
          const tokenBefore = (api.getState() as RootState).user?.accessToken
