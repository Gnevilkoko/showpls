...
export class ResponseService {
  ...
  async respondToRequest(user: User, requestId: string, dto: { message?: string }): Promise<any> {
    ...
    if (String(request.customer.id) === String(user.id)) {
    ...
    if (
      existingResponse &&
      existingResponse.status !== ResponseStatus.Cancelled &&
      existingResponse.status !== ResponseStatus.Rejected
    ) {
  ...
  async acceptResponse(user: User, responseId: string, dto: AcceptResponseDto) {
    return this.dataSource.transaction(async (manager) => {
      ...
      // 3. Who can accept:
      // - regular offer: customer accepts performer's response
      // - direct offer: performer accepts customer's offer
      const isDirectOffer = Boolean((response.request.metadata as Record<string, unknown> | null)?.directOffer === true)
      const canAccept = isDirectOffer
        ? String(response.performer.id) === String(user.id)
        : String(response.request.customer.id) === String(user.id)
      if (!canAccept) {
        throw new ForbiddenException(
          isDirectOffer ? "Only the performer can accept direct offers" : "Only the customer can accept responses"
        )
      }
      ...
      const acceptedNotifyUserId = isDirectOffer ? String(response.request.customer.id) : String(response.performer.id)
      await this.notificationService.send(acceptedNotifyUserId, "response_accepted", {
        dealId: savedDeal.id,
        requestId: savedDeal.request.id,
        text: isDirectOffer
          ? `Your direct offer for "${response.request.title || "Request"}" has been accepted by performer.`
          : `Your response to request "${response.request.title || "Request"}" has been accepted!`,
      })
  ...
  async rejectResponse(user: User, responseId: string, dto: RejectResponseDto) {
    return this.dataSource.transaction(async (manager) => {
      ...
      const isDirectOffer = Boolean((response.request.metadata as Record<string, unknown> | null)?.directOffer === true)
      const canReject = isDirectOffer
        ? String(response.performer.id) === String(user.id)
        : String(response.request.customer.id) === String(user.id)
      if (!canReject) {
        throw new ForbiddenException(
          isDirectOffer ? "Only the performer can reject direct offers" : "Only the customer can reject responses"
        )
      }
      ...
      const rejectedNotifyUserId = isDirectOffer ? String(response.request.customer.id) : String(response.performer.id)
      await this.notificationService.send(rejectedNotifyUserId, "response_rejected", {
        requestId: response.request.id,
        text: isDirectOffer
          ? `Your direct offer for "${response.request.title || "Request"}" was declined by performer.`
          : `Your response to request "${response.request.title || "Request"}" has been declined.`,
      })
  ...
  async withdrawResponse(user: User, responseId: string) {
    ...
    if (String(response.performer.id) !== String(user.id)) {
