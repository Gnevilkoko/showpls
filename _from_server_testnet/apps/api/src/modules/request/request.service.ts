...
export class RequestService {
  ...
  private async createRequestInTransaction(
    ...
    const requestMetadata = {
      ...
      ...(isDirect
        ? {
            directOffer: true,
            directOfferPerformerId: performer ? String(performer.id) : null,
          }
        : {}),
    }
  ...
  async createDirect(user: User, dto: CreateDirectRequestDto): Promise<any> {
    ...
    if (String(performer.id) === String(user.id)) {
      throw new BadRequestException("PERFORMER_UNAVAILABLE")
    }
    ...
    const { savedRequest, attachments, response } = await this.dataSource.transaction(async (manager) => {
      return this.createRequestInTransaction(
        dto,
        user,
        attachmentHashes,
        expiresAt,
    ...
    // После commit: чат/очередь не должны держать транзакцию и блокировать пул соединений
    ...
    try {
      ...
      this.logger.error(
        `createDirect: chat/notify after commit failed for request ${savedRequest.id}: ${
