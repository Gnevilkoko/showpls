...
const NotificationMessage = ({
    ...
    (message.responseId
      ...
      : chatResponses.find(
          (r) =>
            ...
            (isCustomerMessage || String(r.performer?.id) === String(userId))
        ))
  ...
  const responseIdForWithdraw =
    (!isCustomerMessage && (message.responseId ?? performerPendingResponse?.id ?? linkedResponse?.id)) ??
    effectiveResponseId
  ...
  if (message.variant === "newOffer" && !isCustomerMessage && !responseIdForWithdraw) {
    return null
  }
  // Исполнитель отозвал оффер в этой сессии — карточку скрываем
  if (message.variant === "newOffer" && !isCustomerMessage && isHandledLocally) {
    return null
  }
