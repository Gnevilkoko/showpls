...
export class EscrowRefundService {
  ...
  async refund(params: EscrowRefundParams, manager: EntityManager) {
    ...
    const holdTransaction = await manager.getRepository(Transaction).findOne({
      where: {
    ...
    const releaseTransaction = await manager.getRepository(Transaction).findOne({
      where: {
    ...
    if (releaseTransaction) {
      throw new EscrowExceptions.CanReleaseOnlyHoldTransaction()
    }
...
export type EscrowRefundParams = z.infer<typeof schema>
